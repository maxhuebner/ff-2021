...
```{r}
motw <- c("Tobsenheimi", "Bauer")
```

```{r}
ff_matches %>% 
  filter(manager_winner %in% motw & manager_loser %in% motw) %>% 
  mutate(score_differenz = score_winner - score_loser) %>% 
...
ff_history %>% 
  filter(team_manager %in% motw & team_away_manager %in% motw,
         pos == "BN") %>% 
  arrange(-points) %>% 
  select(player, points, starts_with("team"))
...
ff_history %>% 
  filter(team_manager %in% motw & team_away_manager %in% motw,
         ...
         year >= 2019) %>% 
  arrange(points) %>%
  group_by(pos,team_manager) %>% 
  summarise(pts = mean(points)) %>% 
  arrange(pos)
...
ff_history %>% 
  filter(team_manager %in% motw & team_away_manager %in% motw,
         pos == "FLEX") %>% 
  arrange(points) %>% 
  select(team_manager, player, pos, points, year, week) %>% 
  arrange(points)
```
...
ff_matches %>% 
  group_by(year) %>% 
  filter(week <= 10, manager_winner%in% c("Hubner","Bauer","Tobsenheimi","Ha")) %>% 
  count(manager_winner, sort = T, name="Siege") %>% 
  mutate(Record = str_glue("{Siege}-{10-Siege}")) %>% 
  select("Season" = 1,"Manager"=2,Record) %>% 
  #ungroup() %>% 
  arrange(Season) %>% 
  gt() %>% 
  gt_theme_538() %>% 
  tab_header(md("Nach dem Woche 10..")) %>% 
  gt_highlight_rows(rows = c(8,12), font_weight = "normal", fill="#e87d7d") 
  gtsave(.,"test.html")
